...
import { SCORE_CONFIG, scoreUsesLightText, type CapabilityMatrixWithRows, type Score } from '../types/matrix';
...
function getScoreFont(score: Score): Partial<ExcelJS.Font> {
  ...
  const isLightText = scoreUsesLightText(score);
  return {
...
export async function exportMatrixToExcel(
  ...
  headerRow.values = ['Req #', 'Requirements', 'Experience and Capability', 'Past Performance', 'Comments'];
  ...
  for (let i = 0; i < sortedRows.length; i++) {
    ...
    scoreCell.value = matrixRow.experienceAndCapability !== null ? matrixRow.experienceAndCapability : '';
    if (matrixRow.experienceAndCapability !== null) {
      scoreCell.fill = getScoreFill(matrixRow.experienceAndCapability) as ExcelJS.Fill;
 ...
 */
export function generateExportFilename(companyName: string, date: string): string {
