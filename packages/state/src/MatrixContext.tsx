...
import type { CapabilityMatrix, CapabilityMatrixWithRows, CapabilityMatrixRow, UpdateMatrixRowInput } from '@cmm/core';
  ...
  switch (action.type) {
    ...
    case 'REMOVE_MATRIX':
      return {
        ...
        activeMatrix: state.activeMatrix?.id === action.payload ? null : state.activeMatrix,
      };

    case 'UPDATE_MATRIX_NAME':
      return {
        ...state,
        matrices: state.matrices.map((m) => (m.id === action.payload.id ? { ...m, name: action.payload.name } : m)),
        activeMatrix:
    ...
    case 'REMOVE_ROW':
      ...
      return {
        ...
        activeMatrix: {
          ...state.activeMatrix,
          rows: state.activeMatrix.rows.filter((row) => row.id !== action.payload),
        },
      };
...
export function MatrixProvider({ children }: MatrixProviderProps) {
  ...
  return <MatrixContext.Provider value={{ state, dispatch }}>{children}</MatrixContext.Provider>;
}
